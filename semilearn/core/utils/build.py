...
def get_dataset(args, algorithm, dataset, num_labels, num_classes, data_dir='./data', include_lb_to_ulb=True, exp_type="baseline", mask_ratio=0.0):
    ...
    if dataset == "eurosat":
        ...
        lb_dset, ulb_dset, eval_dset = get_cifar(args, algorithm, dataset, num_labels, num_classes, data_dir=data_dir, include_lb_to_ulb=include_lb_to_ulb, exp_type=exp_type, mask_ratio=mask_ratio)
        test_dset = None
    elif dataset == 'svhn':
